...
const signIn = async ({ auth, email, password, provider, appCheck, analytics }) => {
    let userCredential;
    try {
        if (provider) {
            ...
            const token = await (0, utils_1.getServerToken)(_1.serverTokenUrl, appCheck);
...
const signOut = async (auth, appCheck, analytics) => {
    const uid = auth.currentUser?.uid;
    let redirectUrl = _1.baseUrl;
    if (_1.serverSignOutUrl) {
        const response = await (0, utils_1.postRequest)(_1.serverSignOutUrl, appCheck);
        if (response.ok) {
